...
const PAYMENT_LABEL: Record<string, string> = {
  ...
  partially_refunded: "Estorno parcial", payment_review: "Em revisão",
};
...
const TABS: Array<{ key: string; label: string; match: (o: any) => boolean }> = [
  ...
  { key: "pagos", label: "Pagos", match: (o) => o.payment_status === "approved" },
  { key: "aguardando_pagamento", label: "Aguardando pagamento", match: (o) => o.payment_status === "pending" || o.status === "aguardando_pagamento" || o.order_status === "aguardando_pagamento" },
  { key: "pagamento_recusado", label: "Pagamento recusado", match: (o) => o.payment_status === "rejected" },
  { key: "pagamento_revisao", label: "Em revisão", match: (o) => o.payment_status === "payment_review" || o.order_status === "payment_review" },
  { key: "novo", label: "Novos", match: (o) => o.status === "novo" },
  { key: "em_atendimento", label: "Em atendimento", match: (o) => o.status === "em_atendimento" },
  ...
  { key: "saiu_para_entrega", label: "Enviados", match: (o) => o.status === "saiu_para_entrega" || o.fulfillment_status === "shipped" },
  { key: "entregue", label: "Entregues", match: (o) => ["entregue", "retirado", "finalizado"].includes(o.status) || o.fulfillment_status === "delivered" },
  ...
  { key: "cancelado", label: "Cancelados", match: (o) => o.status === "cancelado" || o.payment_status === "cancelled" },
];
...
export default function AdminOrders() {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    if (status === "saiu_para_entrega") { operationalPatch.fulfillment_status = "shipped"; operationalPatch.delivery_status = "out_for_delivery"; }
    if (["entregue", "retirado", "finalizado"].includes(status)) { operationalPatch.fulfillment_status = "delivered"; operationalPatch.delivery_status = "delivered"; }
    if (status === "cancelado") { operationalPatch.fulfillment_status = "cancelled"; operationalPatch.delivery_status = "cancelled"; }
  ...
  const deliveryAddress = (o: any) => {
    const line = [o.delivery_street, o.delivery_number, o.delivery_complement, o.delivery_neighborhood].filter(Boolean).join(", ");
  ...
  const orderMessage = (o: any) => {
    const items = (o.order_items || []).filter((it: any) => it.item_status !== "removido").map((it: any) => `• ${it.quantity}x ${it.product_name} - ${formatBRL(it.unit_price * it.quantity)}`).join("\n");
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${tab === t.key ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-accent"}`}>
      ...
      <div className="bg-card border rounded-xl shadow-card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-secondary text-left"><tr><th className="p-3">#</th><th className="p-3">Data</th><th className="p-3">Cliente</th><th className="p-3">Tipo</th>{isAdmin && <th className="p-3">Total</th>}{isAdmin && <th className="p-3">Pagamento</th>}<th className="p-3">Separação</th><th className="p-3">Status</th><th></th></tr></thead>
          <tbody>
            {filtered.map((o: any) => {
              ...
              return (
                <tr key={o.id} className="border-t">
                  ...
                  <td className="p-3"><Select value={o.status} onValueChange={(v) => updateStatus(o.id, v)}><SelectTrigger className="h-8 w-[180px]"><SelectValue /></SelectTrigger><SelectContent>{STATUSES.map((s) => <SelectItem key={s} value={s}>{LABEL[s] || s}</SelectItem>)}</SelectContent></Select></td>
                  <td className="p-3 text-right"><Button size="sm" variant="outline" onClick={() => setView(o)}>Ver</Button></td>
                </tr>
              );
            })}
            {filtered.length === 0 && <tr><td colSpan={isAdmin ? 9 : 7} className="p-8 text-center text-muted-foreground">Nenhum pedido neste filtro.</td></tr>}
          </tbody>
        </table>
      </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {view && (
            <Tabs defaultValue="separacao">
              <TabsList className="grid grid-cols-4 w-full"><TabsTrigger value="separacao">Separação</TabsTrigger><TabsTrigger value="resumo">Resumo</TabsTrigger><TabsTrigger value="reembolso">Reembolso</TabsTrigger><TabsTrigger value="historico">Histórico</TabsTrigger></TabsList>
              <TabsContent value="separacao" className="pt-3 space-y-2"><div className="text-xs text-muted-foreground mb-2">Marque o status de cada item conforme separar o pedido.</div>{view.order_items?.map((it: any) => <ItemRow key={it.id} item={it} onStatus={updateItemStatus} onNotes={updateItemNotes} />)}</TabsContent>
              <TabsContent value="resumo" className="space-y-2 text-sm pt-3">
                ...
                {isAdmin && <div className="flex justify-between font-bold border-t pt-2"><span>Total</span><span className="price">{formatBRL(view.total)}</span></div>}
                <div className="flex gap-2 pt-3 border-t"><Button className="flex-1 bg-whatsapp hover:bg-whatsapp/90 text-white" asChild><a href={buildWhatsAppLink(view.customer_phone, orderMessage(view))} target="_blank" rel="noreferrer"><MessageCircle className="h-4 w-4 mr-2" />WhatsApp</a></Button><Button variant="outline" onClick={() => { navigator.clipboard.writeText(orderMessage(view)); toast.success("Mensagem copiada"); }}><Copy className="h-4 w-4 mr-2" />Copiar</Button></div>
              </TabsContent>
              <TabsContent value="reembolso"><RefundPanel order={view} /></TabsContent>
              <TabsContent value="historico" className="pt-3"><OrderHistory orderId={view.id} /></TabsContent>
            </Tabs>
          )}
        </DialogContent>
      </Dialog>
...
function OrderHistory({ orderId }: { orderId: string }) {
  const { data, isLoading } = useQuery({ queryKey: ["order_events", orderId], queryFn: async () => (await supabase.from("order_events").select("*").eq("order_id", orderId).order("created_at", { ascending: false })).data || [] });
  ...
  return <div className="space-y-2 max-h-80 overflow-auto">{data.map((e: any) => <div key={e.id} className="text-xs border rounded p-2"><div className="flex justify-between"><span className="font-semibold">{e.type}</span><span className="text-muted-foreground">{new Date(e.created_at).toLocaleString("pt-BR")}</span></div>{(e.old_status || e.new_status) && <div className="text-muted-foreground">{e.old_status || "—"} → <strong>{e.new_status || "—"}</strong></div>}{e.message && <div>{e.message}</div>}</div>)}</div>;
}
