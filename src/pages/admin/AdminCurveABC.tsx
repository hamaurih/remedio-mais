...
export default function AdminCurveABC() {
    ...
    queryFn: async () => {
      ...
      while (true) {
        const { data, error } = await db
          .from("order_items")
          .select("product_id,product_name,quantity,total,orders!inner(created_at,payment_status)")
      ...
      const classified: AbcRow[] = sorted.map((row) => {
        ...
        return { ...row, share, cumulative, abc };
      });

      return {
        rows: classified,
        totalRevenue,
        totalQuantity: classified.reduce((sum, r) => sum + r.quantity, 0),
        counts: {
          A: classified.filter((r) => r.abc === "A").length,
          B: classified.filter((r) => r.abc === "B").length,
          C: classified.filter((r) => r.abc === "C").length,
        },
      };
    },
  });
  ...
  const visible = useMemo(() => {
    ...
    return filter === "ALL" ? rows : rows.filter((r) => r.abc === filter);
  }, [query.data, filter]);
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-[1480px] mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-primary font-bold text-xs uppercase tracking-[0.16em]">
            <BarChart3 className="h-4 w-4" /> Inteligência de estoque
          </div>
          <h1 className="text-3xl font-black tracking-tight mt-1">Curva ABC</h1>
          <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
            Classificação automática dos produtos pelo faturamento acumulado: A até 80%, B de 80% a 95% e C no restante.
          </p>
        </div>
        <Button variant="outline" onClick={() => query.refetch()} disabled={query.isFetching}>
          <RefreshCw className={`h-4 w-4 mr-2 ${query.isFetching ? "animate-spin" : ""}`} /> Atualizar
        </Button>
      </div>

      <div className="flex flex-wrap gap-2">
        {ranges.map((range) => (
          <Button key={range} variant={days === range ? "default" : "outline"} size="sm" onClick={() => setDays(range)}>
            {range === 365 ? "12 meses" : `${range} dias`}
          </Button>
        ))}
      </div>
      ...
      <Card className="rounded-2xl overflow-hidden">
        ...
        <CardContent className="p-0">
          {query.isLoading ? (
            <div className="p-10 text-center text-muted-foreground">Calculando Curva ABC...</div>
          ) : query.isError ? (
            <div className="p-10 text-center text-destructive">Não foi possível calcular a Curva ABC neste ambiente.</div>
          ) : !visible.length ? (
            <div className="p-10 text-center text-muted-foreground">Ainda não existem vendas aprovadas suficientes neste período.</div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40 text-muted-foreground">
                  <tr>
                    ...
                    <th className="text-right px-4 py-3 font-semibold">Participação</th>
                    <th className="text-right px-4 py-3 font-semibold">Acumulado</th>
                    <th className="text-center px-4 py-3 font-semibold">Classe</th>
                  </tr>
                </thead>
                <tbody>
                  {visible.slice(0, 300).map((row, index) => (
                    <tr key={`${row.productId || row.name}-${index}`} className="border-t hover:bg-muted/20">
                      <td className="px-4 py-3 text-muted-foreground">{index + 1}</td>
                      <td className="px-4 py-3 font-semibold min-w-[260px]">{row.name}</td>
                      ...
                      <td className="px-4 py-3 text-right">{row.share.toFixed(2)}%</td>
                      <td className="px-4 py-3 text-right">{row.cumulative.toFixed(2)}%</td>
                      <td className="px-4 py-3 text-center"><AbcBadge value={row.abc} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <Card className={`rounded-2xl ${className}`}>
      <CardContent className="p-4 md:p-5">
        ...
        <div className="text-xs text-muted-foreground font-semibold mt-3">{title}</div>
        <div className="text-xl md:text-2xl font-black mt-0.5 truncate">{value}</div>
      </CardContent>
    </Card>
  );
}
