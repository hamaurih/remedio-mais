-- Ordenação A-Z operacional: ignora espaços e símbolos antes do nome.
...
BEGIN
  ...
  WITH filtered AS (
    ...
    SELECT *
    FROM filtered
    ORDER BY regexp_replace(lower(name), '^[^[:alnum:]]+', '', 'g') ASC NULLS LAST, name ASC NULLS LAST, updated_at DESC NULLS LAST
