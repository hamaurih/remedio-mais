-- Rotation tiers are business-day windows. Compare calendar dates rather than
-- exact timestamps so a sale on the 90th day stays inside the 90-day tier.
...
select b.*,
  ...
  case
    when b.effective_stock>0 or b.last_sale_at::date>=current_date-30 then 15
    when b.last_sale_at::date>=current_date-90 then 60
    when b.last_sale_at::date>=current_date-180 then 1440
    when b.last_sale_at::date>=current_date-365 then 10080
    else null
  ...
  case
    when b.effective_stock>0 or b.last_sale_at::date>=current_date-30 then coalesce(b.last_trier_sync_at,'epoch'::timestamptz)+interval '15 minutes'
    when b.last_sale_at::date>=current_date-90 then coalesce(b.last_trier_sync_at,'epoch'::timestamptz)+interval '60 minutes'
    when b.last_sale_at::date>=current_date-180 then coalesce(b.last_trier_sync_at,'epoch'::timestamptz)+interval '1 day'
    when b.last_sale_at::date>=current_date-365 then coalesce(b.last_trier_sync_at,'epoch'::timestamptz)+interval '7 days'
    else null
  end as next_sync_at
from base b;
